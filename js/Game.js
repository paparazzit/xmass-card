...
function Game(opt) {
	...
	this.stateOfInventory = function () {
		if (
			this.inventoryState.hat &&
			this.inventoryState.blanket &&
			this.inventoryState.stick &&
			this.inventoryState.lolly &&
			this.inventoryState.bonBon &&
			this.inventoryState.cane &&
			this.logMessages.full
		) {
			this.logSpeak(this.logMessages.full);

			delete this.logMessages.full;
			this.invent_full = true;
		}
		if (
			this.inventoryState.cane &&
			this.inventoryState.lolly &&
			this.inventoryState.bonBon &&
			this.logMessages.candyFull &&
			this.gameStates.placeHat
		) {
			this.logSpeak(this.logMessages.candyFull);
			console.log("candy");
			delete this.logMessages.candyFull;
		}
	};
	...
	this.eatCandy = new Howl({
		...
		onplay: function () {
			console.log("palying", this);
		},
	});
	this.placeItem = new Howl({
		src: ["./assets/sounds/useItem.mp3"],
	...
	this.hitting = new Howl({
		src: ["./assets/sounds/hitting.mp3"],
	...
	this.pickUp = new Howl({
		src: ["./assets/sounds/pickuUpSound.mp3"],
		volume: 1,
	});
	this.win = new Howl({
	...
	this.startGame = function () {
		let text = "";
		text +=
			'<div class="thumb inventory" ><img src="./assets/inventory.png" alt="inventory"/></div>';
	...
	this.addToInventory = function (item) {
		...
		this.speaking(item.getAttribute("data-item"));
	...
	this.checkCandy = function (target) {
		...
		if (this.candyFed === 3) {
			...
			setTimeout(() => {
				this.burp.play();
				if (this.inventoryState.blanket) {
					this.logSpeak(this.logMessages.candy);
				} else {
					this.logSpeak(this.logMessages.blanketMissing);
				}
			}, 900);

			return;
		}
		this.eatCandy.currentTime = 0;
		this.checkCandy.volume = 1;
		this.eatCandy.play();
	};
	...
	this.swingStick = function (mob) {
		...
		this.hitting.currentTime = 0;
		this.hitting.volume = 1;
		this.hitting.play();
	...
	this.manageClicks = function (e) {
		...
		if (click.contains("main-panel")) {
			this.panelClicks++;
		}
		if (click.contains("item_pos") || click.contains("log-bare")) {
			this.logClicks++;
		}
		...
		if (
			this.logClicks == 2 ||
			(this.panelClicks == 2 &&
				this.logMessages.msg0 &&
				!this.gameStates.beatTheLog)
		) {
			this.logSpeak(this.logMessages.msg0);
			delete this.logMessages.msg0;
		}

		if (
			!this.invent_full &&
			!this.gameStates.haveHat &&
			(this.logClicks == 4 || this.panelClicks == 4)
		) {
			this.logSpeak(this.logMessages.msg1);
			delete this.logMessages.msg1;
		}
		if (
		...
		if (this.gameStates.present && e.target.classList.contains("present")) {
			...
			e.target.parentElement.append(this.addFireWork());
		...
		if (
			(this.gameStates.gameEnd =
				true && e.target.classList.contains("scroll_wrapper"))
		) {
			this.paper.play();
	...
	this.addFireWork = function () {
		...
		<img src ='assets/scroll.png' alt="" class='scroll'></div>
		<div class="fireworks">
			<div class="expl" ></div>
			<div class="expl"></div>
			<div class="expl"></div>
	...
	this.speakControl = function (click) {
		console.log(click);
	};
	...
	this.logSpeak = function (msg) {
		...
		this.textBubble.children[0].innerHTML = logText;
	...
	this.hideText = function () {
		...
		this.textBubble.children[0].innerHTML = "";
	};
	this.textBubble.addEventListener("click", this.hideText.bind(this));

	// SPEAKING
	this.speaking = function (item) {
		if (item === "hat") {
			this.logSpeak(this.logMessages.haveHat);
			return;
		}
	};
